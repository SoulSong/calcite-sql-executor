...
import static com.shf.calcite.constant.Constant.PLACE_HOLDER;
...
class EmptyScope implements SqlValidatorScope {
    ...
    private void resolve_(final CalciteSchema rootSchema, List<String> names,
        ...
        for (String schemaName : concat) {
            ...
            // 当前的schemaName为表名，但无法从当前的schema中获取对应的table
            if (entry == null) {
                // 当前的schema预定义的table为占位符，说明当前为动态表
                if (schema.getTableNames().contains(PLACE_HOLDER)) {
                    entry =
                            schema.getTable(schemaName, nameMatcher.isCaseSensitive());
                } else {
